...
### INSERT CODE HERE ###
	max_data = int(np.amax(data))
	pmf = np.zeros(max_data+1)
...
def poisson_pmf(param, x):
    return math.exp(-param)*np.power(param,x)/math.factorial(x)

# Function to compute pmf of Poisson with parameter param for values between 0 and n 
def poisson_pmf_vec(param, n):
    pmf = np.zeros(n+1)
    for i in range(len(pmf)):r e t u r n   math . exp(−param )∗np . power ( param , x ) / math . f a c t o r i a l ( x 
